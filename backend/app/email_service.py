...
def send_ticket_created_email(user: User, ticket: Ticket) -> None:
    ...
    email_from = os.getenv("EMAIL_FROM", "no-reply@helpdesk.local")
    ...
    user_name = user.name if hasattr(user, "name") and user.name else "User"
...
"""
    ...
    try:
        ...
        print(f"Ticket email could not be sent: {e}")
