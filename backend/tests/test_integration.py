...
def test_ticket_lifecycle(client):
    ...
    client.post("/api/auth/register", json={
        "username": "lifecycle_user", "email": "lc@test.com", "password": "pass123",
    })

    # 2. Register an admin (we manually test, so we register then use fixture logic)
    client.post("/api/auth/register", json={
        "username": "lifecycle_admin", "email": "lcadmin@test.com", "password": "admin123",
    })

    # Login as user
    user_token = client.post("/api/auth/login", json={
        "username": "lifecycle_user", "password": "pass123",
    }).json()["access_token"]
    ...
    resp = client.post("/api/tickets/", json={
        "title": "Test", "description": "Desc", "category_id": 999,
...
def test_admin_workflow(client, admin_token, user_token, category, normal_user, admin_user):
    ...
    ticket_resp = client.post("/api/tickets/", json={
        "title": "Integration Bug", "description": "Something broke in integration",
    ...
    assign_resp = client.patch(f"/api/tickets/{ticket_id}/assign",
    ...
    status_resp = client.patch(f"/api/tickets/{ticket_id}/status",
    ...
    priority_resp = client.patch(f"/api/tickets/{ticket_id}/priority",
    ...
    comment_resp = client.post(f"/api/tickets/{ticket_id}/comments",
                               json={"content": "Working on this now."},
                               headers={"Authorization": f"Bearer {admin_token}"})
    assert comment_resp.status_code == 201

    # 7. User adds a comment
    user_comment = client.post(f"/api/tickets/{ticket_id}/comments",
                               json={"content": "Thanks for looking into it!"},
                               headers={"Authorization": f"Bearer {user_token}"})
    assert user_comment.status_code == 201

    # 8. Admin resolves the ticket
    resolve_resp = client.patch(f"/api/tickets/{ticket_id}/status",
    ...
    # 9. Verify comments
    comments = client.get(f"/api/tickets/{ticket_id}/comments",
                          headers={"Authorization": f"Bearer {admin_token}"})
