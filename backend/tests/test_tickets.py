...
def test_create_ticket(client, user_token, category):
    resp = client.post("/api/tickets/", json={
        "title": "Test ticket", "description": "A test issue",
...
def test_list_tickets(client, user_token, category):
    ...
    client.post("/api/tickets/", json={
        "title": "T1", "description": "Desc", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
...
def test_get_ticket(client, user_token, category):
    create = client.post("/api/tickets/", json={
        "title": "T2", "description": "Desc", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
    tid = create.json()["id"]
...
def test_filter_tickets_by_status(client, user_token, category):
    client.post("/api/tickets/", json={
        "title": "Open", "description": "D", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
...
def test_update_ticket_status(client, admin_token, user_token, category):
    create = client.post("/api/tickets/", json={
        "title": "T3", "description": "D", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
    tid = create.json()["id"]

    resp = client.patch(f"/api/tickets/{tid}/status", json={"status": "in_progress"},
...
def test_assign_ticket(client, admin_token, user_token, category, admin_user):
    create = client.post("/api/tickets/", json={
        "title": "T4", "description": "D", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
    tid = create.json()["id"]

    resp = client.patch(f"/api/tickets/{tid}/assign",
...
def test_delete_ticket_admin(client, admin_token, user_token, category):
    create = client.post("/api/tickets/", json={
        "title": "T5", "description": "D", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
    tid = create.json()["id"]

    resp = client.delete(f"/api/tickets/{tid}",
                         headers={"Authorization": f"Bearer {admin_token}"})
...
def test_delete_ticket_user_forbidden(client, user_token, category):
    create = client.post("/api/tickets/", json={
        "title": "T6", "description": "D", "category_id": category.id,
    }, headers={"Authorization": f"Bearer {user_token}"})
    tid = create.json()["id"]
